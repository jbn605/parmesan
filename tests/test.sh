...
envs="BUILD_TYPE=${BUILD_TYPE} RUST_LOG=${LOG_TYPE}"
fuzzer="../bin/fuzzer"
input="./input"
output="./output"
...
gclang -O0 -g ${target}.c -lz -o ${target}
get-bc ${target}
...
USE_FAST=1 ${bin_dir}/angora-clang ${target}.bc -fsanitize=address -lz -o ${target}.fast
#USE_FAST=1 ${bin_dir}/angora-clang ${target}.bc -fsanitize=address -S -emit-llvm -lz -o ${target}.fast.ll
USE_TRACK=1 ${bin_dir}/angora-clang ${target}.bc -lz -o ${target}.taint
#USE_TRACK=1 ${bin_dir}/angora-clang ${target}.bc -S -emit-llvm -lz -o ${target}.taint.ll
echo "Compile Done.."
...
cmd="$envs $fuzzer -M 0 -A -c targets.json -i $input -o $output -j $num_jobs"
if [ $MODE = "llvm" ]; then
